...
WARNING: FINAL_VAR requires the variable to exist! Create it first:
...
Then in NEXT response: FINAL_VAR(my_answer)
...
def timeout_context(seconds: int):
    ...
    Works on Unix (using signals) and degrades gracefully on Windows.
    """
    if seconds <= 0:
        yield
        return
    ...
    if hasattr(signal, 'SIGALRM'):
        ...
        # Could implement with threading but adds complexity
        yield
...
# ============================================================================
# TOOL: RLM Query (PRIMARY INTERFACE)
# ============================================================================
...
def rlm_query(
    ...
    The LM can:
    - Write Python code to examine and process context
    - Call llm_query() recursively for sub-tasks
    - Use llm_query_batched() for concurrent processing
    ...
    Args:
        ...
        max_iterations: Maximum LM-code iterations (default: 30)
        enable_semantic_search: Expose semantic_search() in REPL (default: True)
    ...
    try:
        return rlm_completion_loop(
            query=query, context=context, ollama_client=ollama_client,
            max_iterations=max_iterations, enable_semantic_search=enable_semantic_search,
            verbose=verbose
        )
    except Exception as e:
...
def rlm_query_file(
    ...
    """
    RLM query with file as context. Convenience wrapper around rlm_query().

    Args:
        query: User's question or task
        file_path: Path to file containing context
        max_iterations: Maximum LM-code iterations
        enable_semantic_search: Expose semantic_search() in REPL
    ...
    try:
        validated_path = context_mgr.validate_file_path(file_path)
        content = context_mgr.read_file_safe(validated_path)
        return rlm_query(query, content, max_iterations, enable_semantic_search, verbose)
    except (FileNotFoundError, PermissionError) as e:
...
if __name__ == "__main__":
    ...
    # Print startup info to stderr (not stdout, which is used for MCP protocol)
    import sys
    print("=" * 80, file=sys.stderr)
    print("Smart Context Manager - TRUE RLM Edition", file=sys.stderr)
    print("=" * 80, file=sys.stderr)
    print(f"Cache directory: {config.CACHE_DIR}", file=sys.stderr)
    print(f"LLM Model: {config.LLM_MODEL}", file=sys.stderr)
    print(f"Embedding Model: {config.EMBEDDING_MODEL}", file=sys.stderr)
    print("=" * 80, file=sys.stderr)
    print("\nRLM CAPABILITIES:", file=sys.stderr)
    print("  [+] REPL environment for programmatic decomposition", file=sys.stderr)
    print("  [+] Recursive llm_query() for sub-task delegation", file=sys.stderr)
    print("  [+] Batched llm_query_batched() for concurrent processing", file=sys.stderr)
    print("  [+] Semantic search integration in REPL", file=sys.stderr)
    print("  [+] Iterative LM-code execution loop", file=sys.stderr)
    print("\nPRIMARY TOOL:", file=sys.stderr)
    print("  -> rlm_query(query, context) - LM decides how to process!", file=sys.stderr)
    print("\nLEGACY TOOLS (still available):", file=sys.stderr)
    print("  -> process_document_recursive() - Fixed strategies", file=sys.stderr)
    print("  -> semantic_search_document() - Direct search", file=sys.stderr)
    print("=" * 80, file=sys.stderr)
    print("\nAligns with: Recursive Language Models (arXiv:2512.24601)", file=sys.stderr)
    print("=" * 80, file=sys.stderr)
